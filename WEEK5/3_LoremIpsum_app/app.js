...
const input1 = document.querySelector(".number-input");

const showParaDiv = document.querySelector(".display-para");
...
generateButton.addEventListener("click",function (){
  ...
  if(isNaN(value) || value<0 || value>9){
    ...
    showParaDiv.innerHTML= ` <p class="result">
    ${text[random]}</p>`  
    ...
    para = para.map((para1,index)=>{
      
      return  `<p class="para-${index+1} result">${para1}  </p>`;
      
    }).join("");
    
    showParaDiv.innerHTML=para;
  }
...
//   for(const i = 0 ; i<input1 ;i++){
     
//     // let para1 = `<p class="para-${i+1}"> ${text[i]}
//     // </p>` ;


//     let para = document.createElement("p");
//     para.classList.add("para-"+ i+1);
//     para.textContent = text[0];
//     para.appendChild(showParaDiv);
